...
exports.fakeBackendProvider = {
    ...
    useFactory: function (backend, options) {
        ...
        backend.connections.subscribe(function (connection) {
            var testUser = { username: 'test', email: 'test', password: 'test' };
            // wrap in timeout to simulate server api call
            setTimeout(function () {
                // fake authenticate api end point
                if (connection.request.url.endsWith('/authenticate') && connection.request.method === http_1.RequestMethod.Post) {
                ...
                if (connection.request.url.endsWith('/users') && connection.request.method === http_1.RequestMethod.Get) {
